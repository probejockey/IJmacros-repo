...
Dialog.create(" Data Sets ");
	...
	Dialog.addRadioButtonGroup("File type: ", file_types,1,2, "PNG");  
Dialog.show();
...
for (k=0; k<datasets; k++) {
	...
	Dialog.create("Plot Options");
  		Dialog.addChoice("Colour:", newArray("Red", "Green", "Blue", "Yellow", "White"),"Yellow");
		...
		Dialog.addSlider("Cross Width or Dot Size:", 1, 10, 4);
  		...
  		Dialog.addCheckbox("Append Comment", true);
  		items = newArray("Number", "Comment", "Both");
  		Dialog.addRadioButtonGroup("Comment Type:", items, 3, 1, "Both");
		...
		if(iprs[l]!=0) {
			...
			for(m=8;m<PTLength;m=m+2){
				...
				if(((x < Ox)&&(x>Mx)) && ((y>Oy)&&(y<My))) {
					...
					//print("original comment is:"+comment[1]+"shortened comment is :"+replace(comment[1],"   ",""));
...
function getIprList(bmpfiles) {
	iprlist = newArray(400);
	iprdata = newArray(400);
