...
//Ver 1.1 increases size of crosses dropped on calibration spots, beeps when a calibration spot is 
//accepted and allows tracking of stage movement by dropping crosses at a defined interval
...
//Open Position Data
Dialog.create(" Stage Tracking");
    Dialog.addCheckbox("Track Stage Movement?", false);
	Dialog.addSlider("Interval between tracking marks:", 5, 60, 10);
Dialog.show();
tracking=Dialog.getCheckbox();
trackint=Dialog.getNumber();
...
do {
    ...
    if (tracking) {
		if((getTime() - time) > trackint) {
			currstg = readStage();
			//print ("reading, time is " + time);
			xypix[0]= (parseFloat(currstg[0])-parseFloat(calcoeffs[0]))/parseFloat(calcoeffs[1]);
			xypix[1]= (parseFloat(currstg[1])-parseFloat(calcoeffs[2]))/parseFloat(calcoeffs[3]);
			drawCrossO(xypix[0],xypix[1],70,20);
			drawCross(xypix[0],xypix[1],70,20);
			time = getTime();
		}
    }	
...
function moveStage(xmov,ymov,calib) {
	...
	stgxmm = xmov*calib[1]+calib[0];
  	...
  	//print("before stgmove" + stgxmm+","+stgymm+ ","+stgzmm);
  	//print("ts_Z = " + ts_Z+ "stgz[0] = "+ stgz[0]);
 
  			if(ts_Z != 0)
  				{ stgzmm=parseFloat(ts_Z); 
  			} else { 
  				stgzmm = 10.30;}
...
function setZts() {
	Dialog.create("Adjust Stage Focus")
		...
		temp = readStage();
		if(adjust) userZ = temp[2];

		//print("adjust = "+adjust+"Z = "+userZ);
		return userZ;
